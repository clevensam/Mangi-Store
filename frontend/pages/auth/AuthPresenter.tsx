...
export function AuthPresenter({
  t, lang, isRegister, email, onEmailChange, password, onPasswordChange,
  displayName, onDisplayNameChange, loading, onSubmit, onToggleMode
}: AuthPresenterProps) {
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-white dark:bg-slate-950">
      ...
      <div className="relative w-full lg:w-[45%] min-h-[45vh] lg:min-h-screen bg-gradient-to-br from-[#f97316] via-[#ea580c] to-[#c2410c] overflow-hidden flex-col items-center justify-center p-8 lg:p-12 hidden lg:flex">
        ...
        <div className="relative z-10 text-center text-white">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="text-white/90 text-lg lg:text-xl font-medium max-w-sm mx-auto leading-relaxed">
            {lang === 'en' ? 'Modern Point-of-Sale System for Smart Retail Businesses' : 'Mfumo wa Kisasa wa Mauzo kwa Biashara Smart'}
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="mt-10 space-y-3">
            {[
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-10 shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-slate-100 dark:border-slate-800">
            <div className="flex flex-col items-center mb-8">
              ...
              <p className="text-slate-400 dark:text-slate-500 text-sm font-medium">
                {isRegister ? (lang === 'en' ? 'Start your business journey' : 'Anza safari yako ya biashara') : (lang === 'en' ? 'Access your retail intelligence' : 'Fikia akili yako ya biashara')}
              </p>
            </div>
            ...
            <form onSubmit={onSubmit} className="space-y-6">
              {isRegister && (
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" required={isRegister} className="w-full h-14 pl-12 pr-4 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl font-bold text-slate-800 dark:text-white outline-none focus:ring-4 focus:ring-brand-primary/5 focus:border-brand-primary/20 transition-all text-sm" placeholder="Salama Shop" value={displayName} onChange={e => onDisplayNameChange(e.target.value)} />
                  </div>
                </div>
              )}
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input type="password" required className="w-full h-14 pl-12 pr-4 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl font-bold text-slate-800 dark:text-white outline-none focus:ring-4 focus:ring-brand-primary/5 focus:border-brand-primary/20 transition-all text-sm" placeholder="****" value={password} onChange={e => onPasswordChange(e.target.value)} />
                </div>
              </div>

              <button type="submit" disabled={loading} className="w-full h-14 bg-brand-primary text-white rounded-2xl font-black uppercase tracking-widest text-[11px] shadow-xl shadow-orange-200 dark:shadow-none hover:bg-orange-600 active:scale-[0.98] transition-all flex items-center justify-center gap-2 group disabled:opacity-50">
                {loading ? (
            ...
            <div className="mt-8 pt-8 border-t border-slate-50 dark:border-slate-800 text-center">
              <p className="text-sm text-slate-500 dark:text-slate-400">
                {isRegister ? (lang === 'en' ? 'Already have an account?' : 'Tayari una akaunti?') : (lang === 'en' ? "Don't have an account?" : 'Huna akaunti?')}
                <button type="button" onClick={onToggleMode} className="ml-1 text-brand-primary font-semibold hover:text-orange-600 transition-colors">
                  {isRegister ? (lang === 'en' ? 'Sign in' : 'Ingia') : (lang === 'en' ? 'Create account' : 'Fungua akaunti')}
                </button>
              </p>
            </div>
          </div>
