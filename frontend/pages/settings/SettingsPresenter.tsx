...
interface SettingsPresenterProps {
  ...
  onShowAddStaff: (v: boolean) => void;
  staffForm: { displayName: string; email: string; password: string; role: string };
  onStaffFormChange: (d: any) => void;
  onCreateStaff: () => void;
  onToggleStatus: (staff: StaffMember) => void;
  onDeleteStaff: (id: string) => void;
}

function SettingsRow({ icon, label, value, badge }: { icon: React.ReactNode; label: string; value?: string; badge?: React.ReactNode }) {
  return (
...
export function SettingsPresenter({
  t, lang, profile, can, staffMembers, staffLoading,
  showAddStaff, onShowAddStaff, staffForm, onStaffFormChange,
  onCreateStaff, onToggleStatus, onDeleteStaff
        ...
        <div className="max-w-6xl mx-auto">
              ...
              <div className="pt-10 pb-8 px-6 sm:px-8 lg:px-10 flex flex-col sm:flex-row items-center gap-6">
                ...
                <div className="text-center sm:text-left">
                  ...
                  <div className="flex items-center justify-center sm:justify-start gap-3 mt-4">
                    ...
                    <span className={cn("px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1.5",
          ...
          <div className="w-full max-w-full">
            <SettingsSection icon={<User size={18} />} title={t.accountSettings}>
              <SettingsRow icon={<User size={18} />} label={t.yourName} value={profile?.displayName || '-'} />
              <SettingsRow icon={<Mail size={18} />} label={t.emailAddress} value={profile?.email || '-'} />
              <SettingsRow icon={<Shield size={18} />} label="Role" badge={
              ...
              <SettingsRow icon={<CheckCircle size={18} />} label={t.status} badge={
                  ...
                  {staffLoading ? (
                    ...
                    <div className="space-y-3">
                      {staffMembers.map((staff) => (
                        <div key={staff.id} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 transition-all hover:border-slate-200 dark:hover:border-slate-600">
                          ...
                          <div className="flex items-center gap-2 shrink-0 ml-3">
                            <span className={cn("px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider",
                              staff.role === 'manager' ? "bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400" : "bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300"
                            )}>
            ...
            <div className="fixed inset-0 z-[70] overflow-y-auto pointer-events-none flex items-center justify-center p-4">
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => onShowAddStaff(false)} className="p-3 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-2xl text-slate-400 transition-all">
                    <X size={20} />
                  </button>
                </div>
                ...
                <div className="space-y-5">
                  <div className="space-y-2">
                    ...
                    <input type="text" className="w-full h-14 px-5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-2xl focus:ring-4 focus:ring-brand-primary/5 focus:border-brand-primary/30 transition-all font-bold text-slate-800 dark:text-slate-100 text-base outline-none" placeholder="John Doe" value={staffForm.displayName} onChange={e => onStaffFormChange({ ...staffForm, displayName: e.target.value })} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] ml-1">{t.staffEmail}</label>
                    <input type="email" className="w-full h-14 px-5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-2xl focus:ring-4 focus:ring-brand-primary/5 focus:border-brand-primary/30 transition-all font-bold text-slate-800 dark:text-slate-100 text-base outline-none" placeholder="john@shop.co" value={staffForm.email} onChange={e => onStaffFormChange({ ...staffForm, email: e.target.value })} />
                  </div>
                  <div className="space-y-2">
                    ...
                    <input type="password" className="w-full h-14 px-5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-2xl focus:ring-4 focus:ring-brand-primary/5 focus:border-brand-primary/30 transition-all font-bold text-slate-800 dark:text-slate-100 text-base outline-none" placeholder="••••••••" value={staffForm.password} onChange={e => onStaffFormChange({ ...staffForm, password: e.target.value })} />
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={() => onStaffFormChange({ ...staffForm, role: 'manager' })}
                        className={cn("px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all border",
                      ...
                      <button
                        onClick={() => onStaffFormChange({ ...staffForm, role: 'cashier' })}
                        className={cn("px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all border",
                ...
                <div className="pt-4 border-t border-slate-50 dark:border-slate-800 flex gap-3">
                  <button onClick={() => onShowAddStaff(false)} className="flex-1 h-14 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-black text-[10px] uppercase tracking-[0.2em] hover:bg-slate-200 dark:hover:bg-slate-700 transition-all active:scale-[0.98]">
                    {lang === 'en' ? 'Cancel' : 'Ghairi'}
                  </button>
                  <button onClick={onCreateStaff} className="flex-[2] h-14 bg-brand-primary text-white font-black text-[10px] uppercase tracking-[0.2em] rounded-2xl shadow-xl shadow-orange-100 dark:shadow-none hover:bg-orange-600 active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                    <Save size={18} />
                    {t.createStaff}
                  </button>
                </div>
