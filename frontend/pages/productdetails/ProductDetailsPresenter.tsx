...
export function ProductDetailsPresenter({
  t, lang, productId, onBack, product, sales, chartData, loading, logs, totalSold, totalRevenue, profitPerUnit
            ...
            <div className="flex-1 min-h-[200px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{fill: 'currentColor', fontSize: 10, fontWeight: 700}} className="text-slate-400 dark:text-slate-600" dy={10} />
                  <YAxis hide />
                  <Tooltip cursor={{stroke: '#f97316', strokeWidth: 2}} contentStyle={{ borderRadius: '16px', border: 'none', backgroundColor: 'var(--tooltip-bg, #1e293b)', color: 'var(--tooltip-text, #f8fafc)', boxShadow: '0 10px 40px rgba(0,0,0,0.1)' }} labelStyle={{fontWeight: 800, color: 'inherit', marginBottom: '4px'}} itemStyle={{color: 'inherit'}} />
                  <Area type="monotone" dataKey="qty" stroke="#f97316" strokeWidth={4} fillOpacity={1} fill="url(#colorQty)" dot={{r: 6, fill: '#f97316', strokeWidth: 2, stroke: '#fff'}} activeDot={{r: 8, strokeWidth: 0}} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
