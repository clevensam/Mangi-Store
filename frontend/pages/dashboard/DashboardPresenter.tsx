...
import {
  TrendingUp, Package, DollarSign, ShoppingCart, Plus,
  PackagePlus, Clock, AlertTriangle, ArrowRight,
} from 'lucide-react';
...
export function DashboardPresenter({ lang, t, data, loading, onNavigate }: DashboardPresenterProps) {
          ...
          <div className="bg-gradient-to-r from-brand-primary to-orange-500 rounded-2xl p-4 sm:p-6 shadow-lg shadow-orange-200/50 dark:shadow-none relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
              <div>
                ...
                <p className="text-white/80 font-medium mt-1 text-xs sm:text-sm">{getCurrentDate()}</p>
              </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-4 sm:p-5 shadow-sm hover:shadow-md transition-shadow">
              ...
              <p className="text-xl sm:text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">
                {formatCurrency(data?.stats.todaySales || 0)}
              </p>
              <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 font-medium mt-1">
                {data?.stats.todayOrderCount || 0} {lang === 'en' ? 'orders' : 'maagizo'}
              </p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-4 sm:p-5 shadow-sm hover:shadow-md transition-shadow">
              ...
              <p className="text-xl sm:text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">
                {formatCurrency(data?.stats.inventoryValue || 0)}
              </p>
              <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 font-medium mt-1">
                {t.inventoryValue}
              </p>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={data?.weeklySales || []}>
                    ...
                    <Tooltip
                      ...
                      contentStyle={{
                        borderRadius: '12px', border: 'none', backgroundColor: '#1e293b',
                        color: '#f8fafc', boxShadow: '0 10px 40px rgba(0,0,0,0.3)', fontSize: '12px', fontWeight: 'bold',
                      }}
                  ...
                  (data?.topProducts || []).map((product, index) => (
                    <div key={product.productId} className="flex items-center gap-4 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-slate-900 dark:text-slate-100 truncate text-sm">{product.productName}</p>
                        <p className="text-xs text-slate-500 dark:text-slate-400">
                          {product.quantity} {lang === 'en' ? 'units sold' : 'vipande'}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="font-black text-brand-primary text-sm">{formatCurrency(product.revenue)}</p>
                      </div>
                    </div>
                  ))
                )}
                ...
                {(data?.recentTransactions || []).length === 0 ? (
                  ...
                  (data?.recentTransactions || []).map((transaction) => (
                    <div key={transaction.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors border border-transparent hover:border-slate-100 dark:hover:border-slate-700">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-slate-900 dark:text-slate-100 text-sm truncate">{transaction.productName}</p>
                      ...
                      <div className="text-right">
                        <p className="font-black text-slate-900 dark:text-slate-100 text-sm">{formatCurrency(transaction.totalPrice)}</p>
                      </div>
                    </div>
                  ))
                )}
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                {(data?.lowStockProducts?.length || 0) > 0 && (
                  <span className="text-[10px] font-bold text-amber-600 bg-amber-50 dark:bg-amber-950/30 px-2 py-1 rounded-full">
                    {(data?.lowStockProducts?.length || 0)} {lang === 'en' ? 'items' : 'vitu'}
                  </span>
                )}
              </div>
              <div className="space-y-3">
                {(data?.lowStockProducts || []).length === 0 ? (
                  ...
                  (data?.lowStockProducts || []).map((product) => (
                    <div key={product.productId} className="flex items-center gap-4 p-3 rounded-xl bg-amber-50/50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-slate-900 dark:text-slate-100 text-sm truncate">{product.productName}</p>
                        <p className="text-xs text-amber-600 dark:text-amber-400 font-medium">
                          {product.quantity} / {product.threshold} {lang === 'en' ? 'remaining' : 'imebaki'}
                        </p>
                      </div>
                      <button className="text-xs font-bold text-brand-primary hover:underline">{t.restock}</button>
                    </div>
                  ))
                )}
              </div>
