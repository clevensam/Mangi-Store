      ...
      <div className="px-4 sm:px-6 lg:px-8 pb-4 shrink-0">
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[1.5rem] sm:rounded-[2rem] border border-slate-200 dark:border-slate-800 p-4 sm:p-6 shadow-sm">
            ...
            <p className={`text-xl sm:text-2xl font-bold mt-1 ${totals.totalProfit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="appearance-none pl-3 sm:pl-4 pr-8 sm:pr-10 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-semibold text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary/20 cursor-pointer"
                    >
                    ...
                    <div className="flex items-center gap-3 mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                      <div className="relative max-w-[200px]">
                        ...
                        <input
                          ...
                          className="w-full pl-9 sm:pl-10 pr-3 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs sm:text-sm font-medium focus:outline-none focus:ring-2 focus:ring-brand-primary/20 text-slate-700 dark:text-slate-300"
                        />
                      </div>
                    </div>
                  </motion.div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                  {filteredProducts.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="py-12 sm:py-16 text-center text-slate-400 dark:text-slate-500 text-xs sm:text-sm">
                        {t.noSalesFound}
                      </td>
                    </tr>
                  ) : (
                    <AnimatePresence mode="popLayout">
                      {filteredProducts.map((item: any, index: number) => (
                        ...
                        >
                          <td className="py-4 sm:py-6 px-3 sm:px-8">
                            <span className="text-xs sm:text-sm font-semibold text-slate-400">{index + 1}</span>
                          </td>
                          <td className="py-4 sm:py-6 px-3 sm:px-6">
                            <span className="font-bold text-sm sm:text-base text-slate-900 dark:text-slate-100">{item.productName}</span>
                          </td>
                          <td className="py-4 sm:py-6 px-3 sm:px-6 text-right font-bold text-slate-700 dark:text-slate-300 tabular-nums text-sm sm:text-base">
                            {item.totalQuantity.toLocaleString()}
                          </td>
                          <td className="py-4 sm:py-6 px-3 sm:px-8 text-right font-black text-brand-primary tabular-nums text-sm sm:text-base">
                ...
                {filteredProducts.length > 0 && (
                  <tfoot>
                    ...
                    <tr className="border-t border-slate-100 dark:border-slate-800">
                      ...
                      <td className={`py-4 sm:py-6 px-3 sm:px-8 text-right text-xs sm:text-sm font-bold tabular-nums ${totals.totalProfit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                        {formatCurrency(totals.totalProfit)}
                      </td>
                    </tr>
                  </tfoot>
